...
(function ()
{
  ...
  let reactInterval: number | null = null;
  let heartButton: HTMLElement | null = null;
  let heartPath: SVGPathElement | null = null;
  ...
  function showFilledHeart(): void
  {
    ...
    // 触发跳动动画
    const heartSvg = heartButton?.querySelector('.koishi-heart-svg');
    if (heartSvg)
    {
  ...
  function reactToPosts(): void
  {
    ...
    try
    {
      ...
      {
        const reactionButton = control.querySelector(
          '.discourse-reactions-actions:not(.has-reacted) .btn-toggle-reaction-like:not([title="请注册或登录以点赞此帖子"])'
        ) as HTMLElement;
