...
// @icon         https://www.google.com/s2/favicons?sz=64&domain=bot.q.qq.com
// @grant        GM_addStyle
// @run-at       document-end
...
// @downloadURL  https://raw.githubusercontent.com/koakuuma/tampermonkey-scripts/refs/heads/main/dist/qq-bot-docs-downloader.js
// @updateURL    https://raw.githubusercontent.com/koakuuma/tampermonkey-scripts/refs/heads/main/dist/qq-bot-docs-downloader.js
// ==/UserScript==
/******/ (() => { // webpackBootstrap
/******/ 	"use strict";
...
(function () {
  ...
  function sanitizeName(name) {
    return String(name || '').replace(/[\\/:*?"<>|]/g, ' ').replace(/\s+/g, ' ').replace(/^\.+|\.+$/g, '').trim().slice(0, 100) || '未命名';
  }
  ...
  function parseSidebarConfig(appText) {
    const sidebarObject = extractBalancedAt(appText, 'sidebar:{"/develop/api-v2/":', '{', '}');
  ...
  async function expandSidebar() {
    let guard = 0;
    while (guard < 80) {
      const arrow = document.querySelector('.sidebar-group.collapsable > .sidebar-heading:not(.open) .arrow');
      if (!arrow) return;
      arrow.click();
      await delay(30);
      guard += 1;
  ...
  function collectSidebarLeaves(container, groups, output) {
    for (const li of Array.from(container.children)) {
      ...
      const section = Array.from(li.children).find(el => el.tagName === 'SECTION' && el.classList.contains('sidebar-group'));
      const link = Array.from(li.children).find(el => el.tagName === 'A' && el.classList.contains('sidebar-link'));
      if (section) {
        const heading = Array.from(section.children).find(el => el.tagName === 'P' && el.classList.contains('sidebar-heading'));
        const childList = Array.from(section.children).find(el => el.tagName === 'UL' && el.classList.contains('sidebar-group-items'));
        const title = heading ? heading.textContent.trim().replace(/\s+/g, ' ') : '';
        if (childList) {
          collectSidebarLeaves(childList, title ? groups.concat(title) : groups, output);
        }
      } else if (link) {
  ...
  function buildDownloadPlanFromDom() {
    ...
    for (const leaf of leaves) {
      ...
      const title = leaf.title || baseHref.split('/').filter(Boolean).pop().replace(/\.html$/, '') || '未命名';
  ...
  function buildDownloadPlanFromApp(pages, sidebarConfig) {
    ...
    const developPages = pages.filter(page => page.relativePath && page.relativePath.startsWith('develop/api-v2/'));
    const pageByRel = new Map(developPages.map(page => [page.relativePath, page]));
    const walked = walkSidebar(sidebarItems, 'develop/api-v2/', {
      leaves: [],
      unknown: []
    });
    const records = [];
    ...
    for (const leaf of walked.leaves) {
      ...
      if (!page) continue;
      const title = page.title || leaf.titleOverride || rel.split('/').pop().replace(/\.md$/, '') || '未命名';
  ...
  function relativePath(fromFile, toFile) {
    ...
    let commonLength = 0;
    while (commonLength < fromDir.length && commonLength < toParts.length && fromDir[commonLength] === toParts[commonLength]) {
      commonLength += 1;
    }
  ...
  function convertInternalLinks(markdown, currentOutputPath, routeMap) {
    return markdown.replace(/\[([^\]]*)\]\((\/wiki\/[^)#?]+)((?:#[^)]*)?)\)/g, (all, text, url, hash) => {
  ...
  function imageMarkdown(img) {
    ...
    if (!src) {
      const dataSrc = img.getAttribute('data-src') || img.getAttribute('data-original') || img.getAttribute('data-lazy-src') || img.getAttribute('data-url');
      if (dataSrc) {
        try {
  ...
  function tableMarkdown(table) {
    ...
    const normalizedRows = rows.map(row => Array.from(row.children).filter(cell => cell.tagName === 'TH' || cell.tagName === 'TD').map(cell => inlineMarkdown(cell).trim().replace(/\|/g, '\\|')));
  ...
  function blockMarkdown(element) {
    ...
    for (const child of element.childNodes) {
      ...
      if (/^H[1-6]$/.test(tag)) {
        ...
        lines.push(tableMarkdown(childElement));
      } else if (tag === 'PRE') {
        lines.push(codeBlockMarkdown(childElement));
      } else if (tag === 'IMG') {
        const imageText = imageMarkdown(childElement);
        ...
        lines.push('---');
      } else if (tag === 'DIV' || tag === 'SECTION' || tag === 'ARTICLE' || tag === 'MAIN' || tag === 'FIGURE') {
        const text = blockMarkdown(childElement).trim();
        if (text) lines.push(text);
      } else {
        const text = blockMarkdown(childElement).trim();
  ...
  function htmlToMarkdown(html, pageUrl) {
    ...
    const content = doc.querySelector('.theme-default-content') || doc.querySelector('main .theme-default-content') || doc.querySelector('.page .theme-default-content') || doc.querySelector('main');
  ...
  async function getAppText() {
    const appSrc = Array.from(document.scripts).map(script => script.src).find(src => /\/assets\/js\/app\.[a-f0-9]+\.js/.test(src));
    if (!appSrc) {
  ...
  async function handleDownload() {
    ...
    try {
      ...
      await mapWithConcurrency(plan.records, CONCURRENCY, async record => {
        ...
        completed = done;
        setStatus(`下载中：${done}/${total}${failCount ? `，失败 ${failCount}` : ''}...`, failCount > 0);
      }, failures);
      ...
      const timestamp = [now.getFullYear(), pad(now.getMonth() + 1), pad(now.getDate()), '_', pad(now.getHours()), pad(now.getMinutes()), pad(now.getSeconds())].join('');
      saveBlob(blob, `${ROOT_FOLDER}_${timestamp}.zip`);
      setStatus(failures.length ? `完成：成功 ${completed - failures.length} 篇，失败 ${failures.length} 篇。` : `完成：已下载 ${completed} 篇文档。`);
    } catch (error) {
